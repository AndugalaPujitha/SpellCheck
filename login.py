...
#     conn.commit()
    ...
    #cursor.fetchall()
    # Check if the username and password match any row in the database
    cursor.execute('SELECT * FROM login_data WHERE Username=? AND email=? AND Password=? ', (Username,email,Password))
...
login_window.iconphoto(False, tk.PhotoImage(file='logo.png'))
